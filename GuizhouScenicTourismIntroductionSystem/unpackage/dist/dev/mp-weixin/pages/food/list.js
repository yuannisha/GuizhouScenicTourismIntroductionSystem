  ...
  methods: {
    ...
    async loadFoodList() {
      if (this.loading || this.noMore)
        return;
      this.loading = true;
      common_vendor.index.__f__("log", "at pages/food/list.vue:102", "当前分类", this.currentCategory);
      try {
        ...
        common_vendor.index.__f__("error", "at pages/food/list.vue:140", e);
      } finally {
        this.loading = false;
      }
    },
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    e: common_vendor.f($data.foodList, (item, index, i0) => {
      return {
        a: item.cover,
    ...
    f: $data.loading
  }, $data.loading ? {} : $data.noMore ? {} : $data.foodList.length === 0 ? {} : {}, {
    g: $data.noMore,
    h: $data.foodList.length === 0,
    i: common_vendor.o((...args) => $options.loadMore && $options.loadMore(...args)),
    j: $data.listHeight + "px"
  });
}
